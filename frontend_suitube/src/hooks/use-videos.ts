...
export function useUploadVideo() {
  ...
  return useMutation({
    ...
    }) => apiClient.uploadVideo(file, metadata, onProgress, thumbnailFile),
    onSuccess: () => {
...
/**
 * Hook to delete a Walrus video
 */
export function useDeleteVideo() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ videoId }: { videoId: string }) => apiClient.deleteVideo(videoId),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['videos'] });
      queryClient.removeQueries({ queryKey: ['video', variables.videoId] });
    },
  });
}

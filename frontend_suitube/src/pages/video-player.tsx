...
export default function VideoPlayer() {
  ...
  const { mutate: trackViewMutate } = useTrackView();
  ...
  useEffect(() => {
    ...
    if (trackedViewRef.current === videoId) {
      return;
    }

    trackedViewRef.current = videoId;
    trackViewMutate({
      videoId,
      viewerAddress: account?.address,
    });
  }, [apiVideo?.cid, videoId, account?.address, trackViewMutate]);
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm("Xóa video này khỏi Walrus cache?");
    if (!confirmed) {
      return;
    }
    try {
      await deleteVideo.mutateAsync({ videoId: videoCid });
      toast({
        title: "Đã xóa video",
        description: "Video đã được gỡ khỏi danh sách tạm thời.",
      });
      setLocation("/");
    } catch (err: any) {
      toast({
        title: "Xóa thất bại",
        description: err?.message || "Không thể xóa video, thử lại sau.",
        variant: "destructive",
      });
    }
  };
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                >
                  ...
                  <span>{deleteVideo.isPending ? "Deleting..." : "Delete"}</span>
                </Button>
              </div>
            </div>
